...
echo "cd backend && bash run_full_pipeline.sh idkwhatissmyname --sync-only--jwt-token \"$JWT_TOKEN\" --require-jwt --auth-service-url ${AUTH_SERVICE_URL}"
echo ""

echo -e "${YELLOW}Option 2: Save to file and reference${NC}"
echo "echo '$JWT_TOKEN' > /Users/risha/Documents/GitHub/DellInnovate2026_Final/jwt_token.txt"
echo "cd backend && bash run_full_pipeline.sh idkwhatissmyname --sync-only --jwt-token-file /Users/risha/Documents/GitHub/DellInnovate2026_Final/jwt_token.txt --require-jwt --auth-service-url ${AUTH_SERVICE_URL}"
echo ""
