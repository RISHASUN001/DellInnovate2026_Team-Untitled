...
Write-Host "🐳 Auth Service (Docker): http://localhost:8001" -ForegroundColor Cyan
...
try {
    ...
    Get-Process -Name powershell -ErrorAction SilentlyContinue | Where-Object {$_.MainWindowTitle -match "case-service|chatbot-service|mcp-service|auth-service|scraper-service|backend-api|image-service|api-gateway"} | Stop-Process -Force
