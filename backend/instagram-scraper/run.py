...
async def run():
    ...
    user = await instagram.scrape_user("chrishemsworth")
    output.joinpath("user.json").write_text(json.dumps(user, indent=2, ensure_ascii=False), encoding='utf-8')

    post_video = await instagram.scrape_post("https://www.instagram.com/p/DUtOYbyiKCQ/?img_index=1")
    output.joinpath("video-post.json").write_text(json.dumps(post_video, indent=2, ensure_ascii=False), encoding='utf-8')

    post_multi_image = await instagram.scrape_post("https://www.instagram.com/p/DUtOYbyiKCQ/?img_index=1")
    ...
    posts_all = []
    async for post in instagram.scrape_user_posts("chrishemsworth", max_pages=3):
