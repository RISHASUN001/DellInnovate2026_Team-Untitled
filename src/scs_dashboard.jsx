...
async function fetchCaseSignals(caseUser) {
  try {
    const response = await fetch(
      `${API_BASE_URL}/analytics/dashboard/case/${caseUser}/signals`,
    );
    if (!response.ok) throw new Error("Failed to fetch signals");
    const data = await response.json();
    return data.signals || [];
  } catch (error) {
    console.error("Error fetching case signals:", error);
    return [];
  }
}
...
async function fetchAllSignals() {
  try {
    const response = await fetch(`${API_BASE_URL}/analytics/signals`);
...
const MOCK_CASES = [
  {
    ...
    category: "Bullying",
    platform: "Instagram",
    lastSignal: "2026-02-03 06:12 AM",
    status: "Active",
    assignedTo: "Sarah L.",
    assignedToMe: true,
    youth: {
      ...
      avatar: "🧑‍🦱",
      handle: "@emma_chen_15",
      instagramUrl: "https://instagram.com/emma_chen_15",
    },
    signals: [
      "Repeated negative comments in DMs detected",
      "Sentiment shift: positive → negative over 72h",
      "Keyword cluster: isolation, worthless, alone",
    ],
    summary:
      "Adolescent showing linguistic markers consistent with peer-directed harassment. Sentiment analysis flagged a sharp downward shift over three days.",
  },
  {
    id: 2,
    code: "YD-2026-0411",
    ...
    platform: "Instagram",
    lastSignal: "2026-02-03 06:08 AM",
    status: "Escalated",
    assignedTo: "Sarah L.",
    assignedToMe: true,
    youth: {
      ...
      avatar: "👦",
      handle: "@aiden.tan",
      instagramUrl: "https://instagram.com/aiden.tan",
    },
    signals: [
      "High-frequency distress keywords detected",
      "Cross-platform signal correlation with Reddit activity",
      "Temporal pattern: late-night clustering (11 PM – 2 AM)",
    ],
    summary:
      "Multiple high-risk signals across platforms. Late-night activity pattern with elevated distress language. Flagged for urgent review.",
  },
  {
    id: 3,
    code: "YD-2026-0409",
    ...
    platform: "Instagram",
    lastSignal: "2026-02-03 00:15 AM",
    status: "Active",
    ...
    youth: {
      ...
      avatar: "👧",
      handle: "@sophie_lim",
      instagramUrl: "https://instagram.com/sophie_lim",
    },
    signals: [
      "Decreased posting frequency over 30 days",
      "Shift to passive consumption behaviour",
      "Withdrawal from group interactions",
    ],
    summary:
      "Gradual disengagement pattern noted. Posting frequency has declined significantly; interaction with peers has dropped.",
  },
];
...
function getChatbotResponse(input, attachedCase) {
  ...
  let prefix = "";
  if (attachedCase)
    prefix = `📎 *Reviewing case ${attachedCase.code} (${attachedCase.category}, Risk ${attachedCase.riskLevel}/5):*\n\n`;
...
const ONBOARDING_STEPS = [
  ...
  },
  {
    ...
    desc: "The chatbot at the bottom-right offers guidance based on SCS protocols. Use the 📎 icon to attach one of your assigned cases for context. Try the quick-action preset buttons for common questions. Remember: AI guides, you decide.",
    target: "chatbot-area",
    img: "💬",
    step: 12,
    total: 12,
...
export default function App() {
  ...
  const [allCases, setAllCases] = useState(MOCK_CASES);
    ...
    async function loadData() {
      ...
      try {
        ...
        if (cases && cases.length > 0) {
          // Mark first few cases as assigned to current user for demo
          const processedCases = cases.map((c, idx) => ({
            ...c,
            assignedToMe: idx < 3,
            assignedTo: idx < 3 ? "Sarah L." : c.assignedTo || "—",
          }));
          setAllCases(processedCases);
          setAssignedCasesOrder(
            processedCases.filter((c) => c.assignedToMe).map((c) => c.id),
          );
        } else {
  ...
  useEffect(() => {
    ...
    if (onboardingStep === 3) {
      setActiveTab("all");
      setSelectedCase(allCases[1]);
    } // Show case to highlight risk badge
    if (onboardingStep === 4) {
      setActiveTab("all");
      setSelectedCase(allCases[1]);
    } // Show case to highlight status badge
    ...
    if (onboardingStep === 8) {
      ...
      setSelectedCase(myAssignedCases[0]);
    } // Youth profile
    if (onboardingStep === 9) {
      setActiveTab("assigned");
      setSelectedCase(myAssignedCases[0]);
  ...
  return (
    ...
    >
      ...
      >
        <span>🔒</span> <strong>Privacy Notice:</strong> This dashboard displays
        AI-generated risk assessments only. Original social media content is
        never stored or displayed. All outreach decisions are made by you. Your
        edits and interactions are private to your account.
      </div>
      ...
      {stats && !loading && (
        ...
        >
          ...
          <span>{stats.total_signals || 0} signals</span>
          <span>·</span>
          <span>{stats.total_profiles || 0} profiles</span>
          <span>·</span>
          <span
            style={{ color: stats.high_risk_cases > 0 ? "#dc2626" : "#065f46" }}
          >
            {stats.high_risk_cases || 0} high-risk cases
          </span>
          <span>·</span>
          <span>Avg risk: {stats.average_risk_score?.toFixed(2) || 0}</span>
        </div>
      )}
              ...
              <div style={{ padding: "18px 20px 20px" }}>
                ...
                >
                  ...
                  <div style={{ fontSize: 12, color: "#475569" }}>
                    ...
                    {step.step === 2 &&
                      "Cases organized into category columns (Self-Harm, Bullying, etc.), with highest priority cases at the top of each column. Risk badges (green → red) and the 🔄 data freshness indicator top-right."}
                    {step.step === 3 &&
      ...
      <div style={{ flex: 1, display: "flex", overflow: "hidden" }}>
        ...
        <div
          id="workspace-panel"
          style={{
            width: selectedCase ? 380 : "100%",
            minWidth: selectedCase ? 340 : undefined,
            maxWidth: selectedCase ? 420 : undefined,
            background: "#fff",
          ...
          >
            ...
            <button
              ...
              style={{
                ...
                color: activeTab === "assigned" ? "#6366f1" : "#64748b",
                borderBottom:
                  activeTab === "assigned"
                    ? "3px solid #6366f1"
                    : "3px solid transparent",
                  ...
                  {casesByCategory[category].map((c) => (
                    <CaseCard
                      ...
                      onClick={(c) => {
                        if (!c.assignedToMe)
                          return alert(
                            "📌 This case is not assigned to you. Only summary view is available.\\n\\nTo access full details, the case must be assigned to you.",
                          );
                        setSelectedCase(c);
                      }}
                    />
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
                ...
                <div>
                  ...
                  <div style={{ fontSize: 12, color: "#64748b", marginTop: 2 }}>
                    📱 {selectedCase.platform} · 🕐 Last signal:{" "}
            ...
            >
              ...
              <div style={{ flex: 1, padding: 20, overflowY: "auto" }}>
                ...
                >
                  ...
                  <div
                    style={{ fontSize: 12, color: "#4338ca", lineHeight: 1.5 }}
                  >
                    <strong>Privacy Disclaimer:</strong> This view shows
                    AI-generated risk signals only. No raw social media posts,
                    messages, or personal content is stored or displayed. The AI
                    processes anonymised patterns.
                  </div>
                </div>
                ...
                >
                  ...
                  >
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      <button
                        onClick={() =>
                          window.open(selectedCase.youth.instagramUrl, "_blank")
                        }
                        style={{
                          background:
                ...
                >
                  ...
                  >
                    ...
                    <span
                      style={{
                        fontSize: 22,
                        fontWeight: 800,
                        color: RISK_COLORS[selectedCase.riskLevel].text,
                      }}
                    >
                    ...
                    {[1, 2, 3, 4, 5].map((i) => (
                      <div
                        ...
                        style={{
                          flex: 1,
                          background:
                            i <= selectedCase.riskLevel
                              ? RISK_COLORS[selectedCase.riskLevel].text
                              : "#e2e8f0",
                          borderRadius: 5,
                        }}
                      ></div>
                  ...
                  <div style={{ fontSize: 11, color: "#94a3b8", marginTop: 6 }}>
                    {RISK_COLORS[selectedCase.riskLevel].label} risk · Category:{" "}
                ...
                {/* NLP Metrics Overview */}
                {selectedCase.nlp_metrics && (
                  <div
                    style={{
                      background: "#fff",
                      borderRadius: 12,
                  ...
                  >
                    ...
                    >
                      ...
                      >
                        ...
                        >
                          {selectedCase.nlp_metrics.total_signals}
                        </div>
                      ...
                      <div
                        style={{
                          background:
                            selectedCase.nlp_metrics.distress_count > 0
                              ? "#fef2f2"
                              : "#f8fafc",
                          padding: 12,
                          borderRadius: 8,
                      ...
                      >
                        ...
                        >
                          {selectedCase.nlp_metrics.distress_count}
                        </div>
                        <div style={{ fontSize: 11, color: "#64748b" }}>
                          Distress Signals
                        </div>
                      ...
                      <div
                        style={{
                          background:
                            selectedCase.nlp_metrics.distortion_count > 0
                              ? "#fef3c7"
                              : "#f8fafc",
                          padding: 12,
                          borderRadius: 8,
                      ...
                      >
                        ...
                        >
                          {selectedCase.nlp_metrics.distortion_count}
                        </div>
                        <div style={{ fontSize: 11, color: "#64748b" }}>
                          Distortions
                        </div>
                    ...
                    <div style={{ marginTop: 12, display: "flex", gap: 16 }}>
                      <div style={{ flex: 1 }}>
                        ...
                        >
                          <div
                            style={{
                              width: `${Math.min(100, Math.max(0, (selectedCase.nlp_metrics.avg_sentiment + 1) * 50))}%`,
                              height: "100%",
                              background:
                                selectedCase.nlp_metrics.avg_sentiment < -0.3
                                  ? "#ef4444"
                                  : selectedCase.nlp_metrics.avg_sentiment > 0.3
                                    ? "#10b981"
                                    : "#6366f1",
                        ...
                        >
                          {selectedCase.nlp_metrics.avg_sentiment.toFixed(2)}
                        </div>
                      </div>
                      <div style={{ flex: 1 }}>
                        <div
                        ...
                        >
                          <div
                            style={{
                              width: `${selectedCase.nlp_metrics.distress_rate * 100}%`,
                              height: "100%",
                              background:
                                selectedCase.nlp_metrics.distress_rate > 0.3
                                  ? "#ef4444"
                                  : selectedCase.nlp_metrics.distress_rate > 0.1
                                    ? "#f59e0b"
                                    : "#10b981",
                              transition: "width 0.3s",
                            }}
                        ...
                        >
                          {(
                            selectedCase.nlp_metrics.distress_rate * 100
                          ).toFixed(1)}
                          %
                        </div>
                      </div>
                    ...
                    {selectedCase.nlp_metrics.emotion_distribution &&
                      Object.keys(selectedCase.nlp_metrics.emotion_distribution)
                        .length > 0 && (
                ...
                >
                  <button
                    ...
                    style={{
                      width: "100%",
                      background: showNlpPanel
                        ? "linear-gradient(135deg, #6366f1, #8b5cf6)"
                        : "#fff",
                      color: showNlpPanel ? "#fff" : "#1e293b",
                      border: "none",
                      padding: "14px 18px",
                      cursor: "pointer",
                      display: "flex",
                  ...
                  >
                    <span>
                      🔬 View Detailed NLP Signals ({caseSignals.length})
                    </span>
                  ...
                  {showNlpPanel && (
                    ...
                    >
                      {loadingSignals ? (
                        ...
                        >
                          No signals found for this case
                        </div>
                      ) : (
                        <table
                          style={{
                            ...
                            {caseSignals.map((signal, idx) => (
                              ...
                              >
                                <td
                                  ...
                                  title={signal.text}
                                >
                                  {signal.text?.substring(0, 60)}
                                  {signal.text?.length > 60 ? "..." : ""}
                                </td>
                                <td
                                  style={{
                                    padding: "8px",
                                ...
                                >
                                  <span
                                    style={{
                                      ...
                                      fontSize: 10,
                                      background:
                                        signal.emotion_label === "sadness"
                                          ? "#fef2f2"
                                          : signal.emotion_label === "fear"
                                            ? "#fef3c7"
                                            : signal.emotion_label === "anger"
                                              ? "#fee2e2"
                                              : signal.emotion_label === "joy"
                                                ? "#d1fae5"
                                                : "#f1f5f9",
                                      color:
                                        signal.emotion_label === "sadness"
  ...
  return (
    <div
      ...
      style={{
        ...
        overflowY: "auto",
        flexShrink: 0,
        border: highlight ? "2px solid #6366f1" : undefined,
    ...
    >
      ...
      >
        ...
        >
          <div
            style={{
              width: `${progress}%`,
              height: "100%",
              background: mandatoryDone ? "#10b981" : "#6366f1",
              borderRadius: 4,
        ...
        <div style={{ fontSize: 11, color: "#64748b", marginTop: 4 }}>
          ...
          {mandatoryDone && "· ✓ All mandatory items done"}
        </div>
      </div>
            ...
            >
              <div
                ...
                style={{
                  ...
                  background: item.done ? "#6366f1" : "#fff",
                  cursor: "pointer",
                  display: "flex",
                  alignItems: "center",
                  justifyContent: "center",
              ...
              >
                <div
                  ...
                  style={{
                    ...
                    borderRadius: 5,
                    border: item.done ? "none" : "2px solid #d1d5db",
                    background: item.done ? "#8b5cf6" : "#fff",
                    cursor: "pointer",
                    display: "flex",
                    alignItems: "center",
                    justifyContent: "center",
...
function ChatbotPanel({ assignedCases, highlight }) {
  ...
  const send = (text) => {
    ...
    setTimeout(() => {
      setMessages((prev) => [
        ...prev,
        { role: "bot", text: getChatbotResponse(t, attachedCase) },
      ]);
    }, 600);
  };
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <div>
            ...
            <div style={{ fontSize: 10, color: "#94a3b8" }}>
              Guidance based on SCS Protocols
            </div>
          </div>
        </div>
      </div>
              ...
              {m.attached && m.role === "user" && (
                <div
                  style={{
                    fontSize: 10,
                    color:
                      m.role === "user" ? "rgba(255,255,255,0.7)" : "#94a3b8",
                    marginBottom: 3,
                  }}
                >
                  📎 {m.attached.code}
        ...
        <div style={{ position: "relative" }}>
          <button
            ...
            style={{
              ...
              justifyContent: "center",
              fontSize: 18,
              color: attachedCase ? "#6366f1" : "#94a3b8",
            }}
            title="Attach a case"
          >
            📎
              ...
              {assignedCases.map((c) => (
                <button
                  ...
                  style={{
                    width: "100%",
                    textAlign: "left",
                    background: attachedCase?.id === c.id ? "#eef2ff" : "none",
                    border: "none",
                    padding: "8px 12px",
                    cursor: "pointer",
                    fontSize: 12,
